...
async def test_error(hass):
    ...
    assert hass.data[DOMAIN].get_entity("binary_sensor.vaillant_error_f152") is not None
...
async def test_error_removed(hass):
    ...
    assert hass.data[DOMAIN].get_entity("binary_sensor.vaillant_error_f152") is None
    assert "binary_sensor.vaillant_error_f152" not in hass.states.async_entity_ids()


async def test_error_created(hass):
    """Test entity is created and removed."""
    system = get_system()
    system.errors = []
