...
class RoomClimate(VaillantClimate):
    ...
    def __init__(self, hub: ApiHub, room: Room, zone: Zone):
        ...
        self._zone = zone
        self._supported_hvac = list(RoomClimate._HA_MODE_TO_VAILLANT.keys())
        self._supported_presets = list(RoomClimate._HA_PRESET_TO_VAILLANT.keys())

    @property
    def hvac_mode(self) -> str:
        """Get the hvac mode based on vaillant mode."""
        active_mode = self.active_mode
        hvac_mode = RoomClimate._VAILLANT_TO_HA[active_mode.current][0]
        if not hvac_mode:
            if (
                active_mode.current
    ...
    def preset_mode(self) -> Optional[str]:
        ...
        """
        return RoomClimate._VAILLANT_TO_HA[self.active_mode.current][1]
...
class ZoneClimate(VaillantClimate):
    ...
    def __init__(self, hub: ApiHub, zone: Zone):
        ...
        self._supported_hvac = list(ZoneClimate._HA_MODE_TO_VAILLANT.keys())
        self._supported_presets = list(ZoneClimate._HA_PRESET_TO_VAILLANT.keys())
    ...
    def hvac_mode(self):
        ...
        hvac_mode = ZoneClimate._VAILLANT_TO_HA[current_mode][0]
        if not hvac_mode:
            if (
                current_mode
                in [
    ...
    def preset_mode(self) -> Optional[str]:
        """Return the current preset mode, e.g., home, away, temp."""
        return ZoneClimate._VAILLANT_TO_HA[self.active_mode.current][1]
